...
public class AdminAction  extends BaseAction {
	...
	public String listUsers(){
		try {
			...
			List<User> allusers = adminManager.listAllUsers(paramsUser,sum); 
			JSONArray districts = new JSONArray();
			Map<String, Integer> map = new HashMap<String, Integer>();
			for(User user:allusers) {
				try {
					String addressjson = HttpUtil.get("http://api.map.baidu.com/geocoder?address="+user.getNick_name()+"&output=json&key=37492c0ee6f924cb5e934fa08c6b1676");
					 ...
					 if(map.get(user.getNick_name())==null) {
						 map.put(user.getNick_name(), 1);
						object.put("name", user.getNick_name());
						object.put("center", lng+","+lat);
						districts.add(object);
					 }else {
